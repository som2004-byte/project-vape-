        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div className="h-96 rounded-md bg-black/40 p-2">
            {product.modelUrl ? (
              ...
              <img src={product.poster} alt={product.name} className="w-full h-full object-contain" />
            )}
          </div>

          <div className="space-y-3">
            <div className="text-yellow-400 font-bold text-2xl">₹{product.price}</div>
            <p className="text-gray-300">{product.short} · {product.puffs} puffs · {product.nicotine}</p>
